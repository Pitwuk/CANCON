...
uint16_t EE_Init(void)
{
	...
	case ERASED:
		...
		{
			/* Erase Page0 */
			FLASH_Erase_Sector(PAGE0_BASE_ADDRESS, FLASH_VOLTAGE_RANGE_3);
...
//			}
	...
	case RECEIVE_DATA:
		if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
		{
			...
			/* Erase Page1 */
			FLASH_Erase_Sector(PAGE1_BASE_ADDRESS, FLASH_VOLTAGE_RANGE_3);
			/* If erase operation was failed, a Flash error code is returned */
			if (HAL_FLASH_GetError() != HAL_OK)
			{
				return HAL_FLASH_GetError();
...
static uint16_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data)
{
	...
	FLASH_Erase_Sector(OldPageAddress, FLASH_VOLTAGE_RANGE_3);
	/* If erase operation was failed, a Flash error code is returned */
	if (HAL_FLASH_GetError() != HAL_OK)
	{
		return HAL_FLASH_GetError();
